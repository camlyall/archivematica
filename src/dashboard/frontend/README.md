...
First, in the TreeController, we inject the SelectedFiles service, and
...
Because the result from calling `File.get` is a *promise*, it's only
...
With one-line arrow functions, it's also possible to omit the braces;
